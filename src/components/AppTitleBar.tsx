 ...
 */
export function AppTitleBar({ inbox = false }: { inbox?: boolean }) {
  const pageTitle = PAGE_META[useAppNav().active].label;
  // The inbox isn't a tab, so it names itself.
  const title = usePathname() === "/portal/inbox" ? "Inbox" : pageTitle;
